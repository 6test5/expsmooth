...
rb = xlrd.open_workbook('dataimport.xlsx')             # Выбор файла Excel
...
for lst in dt:                                         # Рвзбор списка списков в список
    data.extend(lst)
...
for alp in np.arange(0, 2, 0.1):   # Итерация всех допустимых значений параметра альфа с заданным шагом
    z += 1
    alpha_counting(alp)
